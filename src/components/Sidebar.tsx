...
import { LayoutDashboard, Settings, DollarSign, LogOut, Shield, Building2, Menu, X } from 'lucide-react';
...
const Sidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      <div className="fixed left-0 top-0 w-64 bg-gradient-to-b from-blue-600 via-blue-700 to-blue-800 text-white h-screen flex flex-col z-50 shadow-xl transform -translate-x-full lg:translate-x-0 transition-transform duration-300" id="sidebar">
        ...
        <div className="lg:hidden flex justify-end p-4">
          <button
            ...
            className="text-white/70 hover:text-white bg-white/10 p-2 rounded-xl backdrop-blur-sm active:scale-95 transition-all"
        ...
        <nav className="flex-1 p-3 lg:p-4 overflow-y-auto">
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-2 lg:gap-3 p-2 lg:p-3 rounded-xl mb-1 lg:mb-2 text-left transition-all duration-200 ${isActive
          ...
          {tabVisibility.admin && (
            <button
              ...
              className={`w-full flex items-center gap-2 lg:gap-3 p-2 lg:p-3 rounded-xl mb-1 lg:mb-2 text-left transition-all duration-200 ${location.pathname === '/admin'
                  ? 'bg-white/20 text-white shadow-lg backdrop-blur-sm'
                  : 'hover:bg-white/10 text-blue-100 hover:text-white'
                }`}
            >
      ...
      <button
        className="lg:hidden fixed top-4 left-4 z-40 bg-white/90 backdrop-blur-md text-blue-700 p-2.5 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/20 active:scale-95 transition-all focus:outline-none"
        onClick={() => {
          const sidebar = document.getElementById('sidebar');
          const overlay = document.getElementById('sidebar-overlay');
